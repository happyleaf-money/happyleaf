...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zHLF spends in this block or tx
        ...
        if (!fProofOfStake) {
            ...
            if(nHeight > 1) { // exclude premine
				...
				LogPrintf(">>>>>>>> BAG ALERT = mn_reward !fProofOfStake\n");
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HLFMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("HLFMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("HLFMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("happyleaf-miner");
	
	//POS time
	unsigned int nLastPosTime = 0;    
    bool SPORK_21 = true;
	int SPORK_21_VAL = 5; // 5 is optimal 
